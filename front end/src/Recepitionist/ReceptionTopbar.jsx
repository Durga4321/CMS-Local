...
import { receptionModuleSearchItems, searchModuleItems } from "../utils/moduleSearch";
...
function ReceptionTopbar({ title, onMenu, areaLabel = "Reception", roleType = "receptionist" }) {
  ...
  const results = useMemo(() => searchModuleItems(receptionModuleSearchItems, query), [query]);
  const goTo = (path) => {
    setQuery("");
      ...
      <div className="rc-top-actions">
        <form className="rc-search rc-module-search" onSubmit={submitSearch} onBlur={() => window.setTimeout(() => setShowResults(false), 120)}>
          ...
          <input
            ...
            placeholder="Search reception modules..."
          />
          {showResults ? (
            <div className="rc-search-results">
              {results.length ? results.slice(0, 7).map((item) => (
                <button key={item.path + item.label} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => goTo(item.path)}>
                  {item.label}
                </button>
